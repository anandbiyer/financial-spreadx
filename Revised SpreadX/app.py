...
# Anthropic (direct) reads ANTHROPIC_API_KEY from the environment.

from config import ANTHROPIC_MODEL, BEDROCK_DEFAULT_MODEL_ID, get_llm_settings, set_llm_settings
from export.xlsx_export import build_raw_extraction_xlsx
from pipeline.orchestrator import run_pipeline
from spreading.ui_streamlit import render_learning_store, render_spreading_section
...
with st.sidebar:
    ...
    # ── LLM provider toggle ──────────────────────────────────────────────
    # Claude API (direct) now; AWS Bedrock for customer demos. Switching here
    # routes ALL Claude calls (extraction, vision, notes, classification,
    # COA mapping) through the chosen provider — no code change needed.
    _PROVIDER_LABELS = {"Claude API (direct)": "anthropic", "AWS Bedrock": "bedrock"}
    _labels = list(_PROVIDER_LABELS.keys())
    _current_provider = get_llm_settings().get("provider", "anthropic")
    _default_idx = 0 if _current_provider == "anthropic" else 1
    _provider_choice = st.radio(
        "LLM Provider",
        _labels,
        index=_default_idx,
        help="Claude API uses ANTHROPIC_API_KEY from the environment. "
             "AWS Bedrock uses AWS credentials from the environment / ~/.aws.",
    )
    _provider_key = _PROVIDER_LABELS[_provider_choice]
    _default_model = ANTHROPIC_MODEL if _provider_key == "anthropic" else BEDROCK_DEFAULT_MODEL_ID
    _model_id = st.text_input(
        "Model ID",
        value=_default_model,
        key=f"llm_model_{_provider_key}",
    )
    set_llm_settings(provider=_provider_key, model=_model_id)
    st.caption("API keys are read from environment variables, never stored in the UI.")
    ...
    st.caption("Stages: S2 (classify) > S3 (filter) > S5 (extract) > S6 (notes) > S11 (spread)")

# Learning Store admin (sidebar expander) — available regardless of upload state.
render_learning_store()
...
if uploaded is not None:
    ...
    # ── Stage 11: COA Mapping & Spreading ─────────────────────────────────
    render_spreading_section(result, template, uploaded.name)
