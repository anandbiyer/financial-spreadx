...
class PipelineResult:
    ...
    # Stage 11 (COA Mapping & Spreading) — populated only when run_spreading=True.
...
def run_pipeline(
    ...
    Args:
        ...
        filename:          Source filename (recorded with the spread result).
        run_spreading:     If True, run Stage 11 (COA mapping & spreading) after S6.
        llm_settings:      Optional {provider, model, api_key} applied for this run
                           (overrides the global toggle for the duration of the run).

    Returns:
        PipelineResult with all extracted data (and spread_result if requested).
    """
    if llm_settings:
        from config import set_llm_settings
        set_llm_settings(**llm_settings)

    result = PipelineResult(template_type=template_type)

    # Meter LLM token usage for this run (extraction stage by default; S11 flips
    # the stage to "spreading"). Active only within this call's context.
    usage_meter = UsageMeter()
    usage_token = set_active_meter(usage_meter)
    usage_meter.set_stage("extraction")
    ...
    # Stamp a stable 1-based Extraction ID on every row (post-dedup, single source of
    # truth). The same id is written to the extraction workbook AND carried through
    # Stage 11 into the spread workbook, so each spread line traces to its source rows.
    ...
    # ── S11: COA Mapping & Spreading (optional, gated) ───────────────────
    # Off by default so existing extraction behaviour is unchanged. When on,
    # map every extracted row to the standardised CoA, persist the spread
    # result, and surface a summary. Imported lazily so the extraction-only
    # path never requires the spreading/DB dependencies.
    if run_spreading and all_rows:
        _notify(progress_callback, "S11", "COA mapping & spreading...", 0.96)
        from spreading.coa_mapper import run_coa_mapping_stage

        usage_meter.set_stage("spreading")
        scopes = [r.get("statement_scope", "unknown") for r in all_rows]
        doc_scope = max(set(scopes), key=scopes.count) if scopes else "unknown"
        spread_kwargs = {}
        ...
        result.spread_result = spread
        result.summary["spread"] = {
            "document_id": spread["document_id"],
            "status": spread["status"],
            **spread["counts"],
            "balanced": spread["balance_check"].get("isBalanced"),
        }
        _notify(progress_callback, "S11",
